...
from astropy.coordinates import SkyCoord # se togli il test lo puoi eliminare
...
.          For each tile across the sky the file is updated."""""
    ...
    def east(self, shift_up_down=0, shift_right_left=0):
        ...
        for ra_start, dec_start in zip (fov_center_ra, fov_center_dec):              
            ra_distance = self.__ra0ra1((self.user.get_fov_width() - self.SHIFT_CORRECTION + shift_right_left),
    ...
    def west(self, shift_up_down=0, shift_right_left=0):
        ...
        for ra_start, dec_start in zip (fov_center_ra, fov_center_dec):
            ...
            ra_distance = self.__ra0ra1((self.user.get_fov_width() - self.SHIFT_CORRECTION + shift_right_left),float(dec_start), float(dec_start))
...
## small shifts
        ...
        self.B62 = Button(frame,text="✓ Accept",fg ="grey",
                          pady=1) 
...
##################################################
        if event.widget == self.B63:    # ↟
            ...
            # small_shift: adjustments
            adj = ShowSkyCoverage()
            
            entries_GWsky = adj.load_entries("GWsky_entries")        
            fov_center_ra, fov_center_dec = entries_GWsky[0::2], entries_GWsky[1::2]
            
            shift_up_down=-0.1 # set small shift
            ...
            for ra_start, dec_start in zip (fov_center_ra, fov_center_dec):
                ...
                dist = adj.intercardinal_distance(ra_start, dec_start,
                                                    shift_up_down, shift_right_left=0)
                north_pointing = [(dist),
                                   (dec_start + adj.user.get_fov_height() + shift_up_down)]
             
                ra, dec = north_pointing[0], north_pointing[1]
                
                aladin.set_target(ra, dec)
                aladin.set_plane_id("P:"+str(ra) + ',' + str(dec))
                # remove coord, query and in GWsky_entries
                
                new_sky_pos = [ra,dec] # cycle variables
                adj.entries_GWsky_new.extend(new_sky_pos)
                #adj.pick_coverage(ra, dec)
                aladin.remove("Q:"+str(ra_start)+"/"+str(dec_start))
                
                aladin.set_plane_id("P:"+str(ra) + ',' + str(dec))
                
            with open('GWsky_entries', 'wb') as data:
                pickle.dump(adj.entries_GWsky_new, data)
                

        if event.widget == self.B62:
            ...
            for ra_start, dec_start in zip (fov_center_ra, fov_center_dec):
                ...
                adj.pick_coverage(float(ra_start), float(dec_start))
            ...
            #    tkMessageBox.showerror ('Error', value_error)
                ...
                #entries_GWsky.extend(current_fov_coords)
...
class Observability(Toplevel):
    ...
    def moc_obs(self):
        ...
        altaz = sky_coord.transform_to(AltAz(obstime=self.user.get_obs_time(), location=observatory))

        airmass_values = altaz.secz
        print airmass_values
        # creating an astropy.table with RA[deg] and DEC[deg] ipix positions
        contour_ipix = Table([ ra, dec, airmass_values ], names = ('RA[deg]', 'DEC[deg]', 'airmass'), meta = {'ipix': 'ipix table'})
    ...
    def plot_stats(self, time_step, airmass_values, ra, dec, table_q,
        ...
        def subplot_cat_distribution():
            ...
            try:
                ...
                #aladin.select("GWsky_query_items")
                #aladin.set_plane_id("Q:"+str(ra)+"/"+str(dec))
                #aladin.remove('GWsky_query_items')
        ...
        def draw_area():
            ...
            fov_information_title = "FoV center (ra "+str(ra) + "  " +"dec "+ str(dec)+")" + "; " + "prob: " + str(prob_fov)+ "; Moon(illumi.: and dist.:)"    
