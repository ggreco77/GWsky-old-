...
class LoadSkymap(Toplevel):
    ...
    def new_skymap(self):
        ...
        import sys
        try:
            infile  = gzip.GzipFile(self.entry_new_skymap.get(),'rb').read()      
              
            outfile = open("uncompress.fits", 'wb')
            outfile.write(infile)
            outfile.close()
            
            aladin.send_file("uncompress.fits")  # sending to Aladin
        ...
        def update_GWsky_config():
            """Updating GWsky_config file: coodinatas max probability pixel, nside."""
